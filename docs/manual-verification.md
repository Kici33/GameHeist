...
- As a regular player with `heist.play`, run `/heist controls` and check tab completion. Verify it explains the slot 1/2 actions, medkit limits, revive hold, Shift+F loot return, normal F reload, and extraction. Admin permissions should not be required for the control reference.
- As host, use `/heist results` and `/heist results 2` with more than ten retained results. Verify newest-first order, no duplicated entries across pages, exact run times, and `unavailable` for legacy times. Invalid page numbers should report an error; an empty history should show an explicit message.
- Wait in briefing, then start and finish a run. The result's `Run time: M:SS.mmm` must exclude briefing wait and preserve recorded milliseconds. Check the optional `gameplayMillis` field in MongoDB, including an aborted started run. Aborting before start leaves the field absent. `/heist stats` shows the fastest timed win in the same format and selected scope, or `no timed win in available results`. Verify two times in the same second remain distinguishable, and a faster loss/abort or old untimed win cannot replace the record. Memory mode labels the record `Best retained winning run`; MongoDB uses `Best saved winning run`. Evicting a timed win must recalculate the retained best or leave no timed record. Verify saved-mode results again after restart.
- Verify `Time M:SS` on the objective boss bar starts only when the heist starts and follows the arena time limit. In the final minute the bar turns red, including during extraction. At the deadline the run must end with the existing time-limit outcome. Check minute rollover, result cleanup, and readability alongside the responder countdown.
- Trigger the alarm in graybox:4 and check `Responders in ...s` on the objective boss bar. It should count down from 15 seconds, disappear when the single wave is claimed, and never restart for that match. In the final five seconds, one short title and chat warning should appear for the present crew only. A delayed frame must display actual remaining seconds, not a fixed five. No countdown or warning should appear in stealth, non-combat arenas, or after match end. Verify actual responder arrival and that aborting early removes the HUD.
