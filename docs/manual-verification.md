...
- Wait in briefing, then start and finish a run. The result's `Run time: M:SS.mmm` must exclude briefing wait and preserve recorded milliseconds. Check the optional `gameplayMillis` field in MongoDB, including an aborted started run. Aborting before start leaves the field absent. `/heist stats` shows the fastest timed win in the same format and selected scope, or `no timed win yet`. Verify two times in the same second remain distinguishable, and a faster loss/abort or old untimed win cannot replace the record. Memory mode records follow retained history; verify saved-mode results again after restart.
