...
- Wait in briefing, then start and finish a run. The result's `Run time: M:SS` must exclude briefing wait. Check the optional `gameplayMillis` field in MongoDB, including an aborted started run. Aborting before start leaves the field absent. `/heist stats` shows the fastest timed win in the selected scope, or `no timed win yet`. Verify a faster loss/abort and an old untimed win cannot replace that record. Memory mode records follow retained history; verify saved-mode results again after restart.
