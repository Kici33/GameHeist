...
- Wait in briefing, then start and finish a run. The result's `Run time: M:SS` must exclude briefing wait. Check the optional `gameplayMillis` field in MongoDB, including an aborted started run. Aborting before start leaves the field absent. Historical results without it remain valid; the feature does not yet aggregate best times in `/heist stats`.
